...
//
// FMPOS papers follow one shape, whoever wrote them:
//
...
/**
 * `gaps` is how far the numbering may jump forward and still be believed. It
 * is 0 for a paper with real text, where a number out of turn is a number
 * inside a sentence. A photographed paper is different: the camera loses a
 * line now and then, and one lost `17.` must not take 18, 19 and 20 with it.
 * A jump is only allowed between questions — the one being built has all its
 * propositions — and only into a capitalised stem.
 */
export function parseQcm(text, { gaps = 0 } = {}) {
  ...
  for (const raw of lines) {
    ...
    // A paper split into `Exercice 1`, `Exercice 2` restarts its numbering, so
    // a `1.` arriving after a question that already has propositions is a new
    // section rather than a stray number in a sentence.
    const restart = Number(qM?.[2]) === 1 && q && q.options.length >= 2;
    // A number the OCR never read leaves a hole. Step over a small one.
    const jumped = gaps > 0 && qM && q && q.options.length >= 2 &&
      Number(qM[2]) > expected && Number(qM[2]) <= expected + gaps &&
      /^[A-ZÀ-Ý]/.test(qM[3]);
    if (qM && (qM[1] || restart || jumped || expected === null || Number(qM[2]) === expected)) {
      closeQuestion();
