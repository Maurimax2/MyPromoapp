...
/**
 * A key set in two columns comes out of the PDF interleaved — `1 C 5 C 2 C 6 B`
 * is 1C 2C 3C 4C down the left and 5C 6B 7C 8A down the right. The numbers are
 * not in order, so the run rule above refuses it. It is still a key, and a
 * complete one: every number from 1 to N present exactly once. That is a
 * strong enough signature to accept.
 */
export function parseColumnKey(text) {
  const flat = clean(text.replace(/\([A-E]\)/g, ''));
  const pairs = [];
  const token = /(\d{1,3})\s*[.:)–-]?\s*([A-E]{1,5})(?![A-Za-zÀ-ÿ])/g;
  let m;
  while ((m = token.exec(flat))) pairs.push([Number(m[1]), m[2]]);

  // Walk backwards from the end: a key sits at the foot of the sheet.
  for (let start = 0; start < pairs.length; start++) {
    const run = pairs.slice(start);
    const numbers = run.map(([n]) => n);
    const max = Math.max(...numbers);
    if (max !== run.length || max < 4) continue;
    if (new Set(numbers).size !== max) continue;
    if (Math.min(...numbers) !== 1) continue;
    return new Map(run.map(([n, l]) => [n, toIndexes(l)]));
  }
  return new Map();
}

/** Whichever of the shapes the document turns out to be. */
export function parseAnswerKey(text) {
  const found = [parseBlockKey(text), parseTableKey(text), parseColumnKey(text)];
  return found.reduce((best, k) => (k.size > best.size ? k : best), new Map());
}
