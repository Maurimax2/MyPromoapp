...
import { numberLectures } from './lectures.js';
...
function assemble(modules, chapters, documents) {
  ...
  for (const m of byModule.values()) {
    ...
    // …and then the numbering, straight through the module. A subject added
    // through the panel carries no `n` on any row, and a lecture drawn with a
    // dash where its number goes is one تصنيف الأسئلة cannot send a question to.
    numberLectures(m.chapters.flatMap((c) => c.lectures));
  }
...
/**
 * Which years' subjects a year studies.
 *
 * FMPOS has three tracks, and their first years share a programme: dental's
 * first year (PCED1) studies medicine's first year (PCEM1) all year, and
 * pharmacy's (PCEP1) for the first semester only. So a year is its own
 * subjects plus, where `promos.reads_from` says so, another year's — in the
 * semesters `reads_semesters` names (all of them when it is empty).
 *
 * Asked once per request. A database that has not had accounts.sql pasted
 * has no such columns, and then a year is simply itself.
 */
export const sourcesOf = cache(async (promo) => {
  ...
  // Before accounts.sql the database cannot say; the file can.
  const row = !error && data ? data : FILE_PROMOS.find((p) => p.id === promo);
  if (!row?.reads_from) return own;
  const semesters = row.reads_semesters?.length ? row.reads_semesters : null;
  return [...own, { promo: row.reads_from, semesters }];
});

/** Whether a subject belongs to what a year studies (see sourcesOf). */
export const studies = (sources, m) =>
  sources.some((s) => s.promo === m.promo && (!s.semesters || s.semesters.includes(m.semester)));

// Subjects read from several years come back in semester order, each year's
// own order kept within a semester.
const bySemester = (a, b) =>
  String(a.semester).localeCompare(String(b.semester)) || (a.position ?? 0) - (b.position ?? 0);
...
export async function modulesOf(promo) {
  const sb = await supabaseServer();
  const sources = await sourcesOf(promo);

  const reads = await Promise.all(sources.map((src) => sb.from('modules')
    .select('id, promo, semester, name, icon, tint, professors, position')
    .eq('promo', src.promo).order('position')));
  const modules = reads.flatMap((r, i) => (r.data || []).filter((m) => studies([sources[i]], m)));
  if (sources.length > 1) modules.sort(bySemester);

  if (!modules.length) return fileFor(sources);
  ...
  const fromFile = new Map(fileFor(sources).map((m) => [m.id, m]));
...
// The file's subjects for what a year studies, the same rule as sourcesOf.
const fileFor = (sources) => sources.flatMap((src) =>
  fileModules(src.promo).filter((m) => studies([src], m)));
...
export async function subjectsOf(promo) {
  ...
  const sources = await sourcesOf(promo);
  const reads = await Promise.all(sources.map((src) => sb.from('modules')
    .select('id, promo, name, semester, icon, tint, position').eq('promo', src.promo).order('position')));
  ...
  if (reads.some((r) => r.error)) return [];

  const data = reads.flatMap((r, i) => (r.data || []).filter((m) => studies([sources[i]], m)));
  if (sources.length > 1) data.sort(bySemester);

  return data.length
    ? data
    : fileFor(sources).map((m) => ({
        id: m.id, promo: m.promo, name: m.name, semester: m.semester, icon: m.icon, tint: m.tint,
      }));
}
 ...
 */
export async function semestersOf(m, viewer = null) {
  if (!m?.promo) return [{ id: m.id, semester: m.semester }];
  const sb = await supabaseServer();
  const { data } = await sb.from('modules')
    .select('id, name, semester, promo').eq('promo', m.promo);

  // A pharmacy first-year reading medicine's anatomy studies its S1 only;
  // the switch must not offer them medicine's S2.
  const sources = viewer ? await sourcesOf(viewer) : null;
  const key = subjectKey(m.name);
  const mine = (data || []).filter((x) => subjectKey(x.name) === key
    && (!sources || !sources.some((s) => s.promo === x.promo) || studies(sources, x) || x.id === m.id));
 ...
 */
//
// Counted once a minute, not once a visit. Walking the whole table a page at
// a time is several round trips in a row, and الدراسة and الرئيسية both
// waited on it before drawing anything — the tab seemed not to open. The
// numbers only change when the panel imports files, and a minute behind is
// a number nobody can tell from the right one. Read with the service key
// because a cached answer cannot depend on whose cookie asked for it.
const countAll = unstable_cache(async () => {
  ...
  // Plain arrays: a cached answer is stored as JSON, and a Map is not.
  return { entries: [...counts], error: error ? error.message : null };
}, ['module-counts'], { revalidate: 60 });

export async function moduleCounts() {
  const { entries, error } = await countAll();
  return { counts: new Map(entries), error };
}
...
/**
 * The years, for somebody who is not in yet — the sign-up form and /waiting.
 *
 * `promos` is readable by approved accounts only, so read as the visitor
 * this came back empty: the sign-up form showed no years to choose and a new
 * account on /waiting had nothing to tap. Which years exist is not a secret,
 * so it is read with the service key, names and nothing else.
 */
export async function yearsForJoining() {
  const db = supabaseAdmin();
  let { data, error } = await db.from('promos')
    .select('id, name, badge, position, track, year').order('position');
  if (error) ({ data, error } = await db.from('promos').select('id, name, badge, position').order('position'));
  if (error || !data?.length) return FILE_PROMOS.map((p) => ({ id: p.id, name: p.name, badge: p.badge }));
  return data;
}
...
export async function promosOf() {
  const sb = await supabaseServer();
  // The track columns come with accounts.sql. Before it is pasted they do not
  // exist, and asking for them would fail the whole read — so ask again
  // without them rather than fall back to the file.
  let { data, error } = await sb.from('promos')
    .select('id, name, label, badge, indexed, position, track, year, reads_from, reads_semesters').order('position');
  if (error) {
    ({ data, error } = await sb.from('promos')
