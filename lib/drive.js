 ...
 *
 * Drive answers failures with a page of JSON. Putting that on screen is worse
 * than useless: it is unreadable, it says nothing about what to do, and it is
 * one unbroken string that pushes the layout off the side of the phone.
 */
export function driveMessage(status, body) {
  const text = String(body || '');
  if (/API key not valid|API_KEY_INVALID/i.test(text)) {
    return 'مفتاح Google غير صالح — تحقّق من GOOGLE_API_KEY في Vercel';
  }
  if (/has not been used|is disabled|SERVICE_DISABLED/i.test(text)) {
    return 'Google Drive API غير مُفعّل لهذا المفتاح — فعّله من Google Cloud Console';
  }
  if (status === 403) return 'المفتاح لا يملك صلاحية القراءة — تحقّق من قيود المفتاح';
  if (status === 404) return 'لم نجد هذا المجلد — تأكد أنه مشارك مع «أي شخص لديه الرابط»';
  if (status === 429) return 'تجاوزنا حدّ الطلبات على Drive — أعد المحاولة بعد دقيقة';
  return `تعذّرت القراءة من Drive (${status})`;
}

/** One file's own details, for a link that points at a file rather than a folder. */
export async function getFile(fileId, key) {
  ...
  if (!res.ok) {
    ...
    const err = new Error(driveMessage(res.status, body));
