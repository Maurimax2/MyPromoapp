...
const withSuppliedAnswer = (moduleId, bank, seen) => (q) => {
  const nth = (seen.get(q.n) || 0) + 1;
  seen.set(q.n, nth);
  const key = `${moduleId}:${bank.fid}:${q.n}${nth === 1 ? '' : `-${nth}`}`;
  if (!q.unanswered && q.answer.length) return { ...q, by: 'paper' };
  const mine = SUPPLIED[key];
  if (!mine) return null;
...
// number. The first keeps the plain id so answers already written stay
// attached; the ones after it are suffixed, so an answer can never land on
// the wrong question.
const withIds = (moduleId, bank) => {
  const seen = new Map();
  const numbering = new Map();
  return bank.questions
    .map(withSuppliedAnswer(moduleId, bank, numbering))
    .filter(Boolean)
    .map((q) => {
      const nth = (seen.get(q.n) || 0) + 1;
      seen.set(q.n, nth);
      const suffix = nth === 1 ? '' : `-${nth}`;
      return { ...q, id: `${moduleId}:${bank.fid}:${q.n}${suffix}`, topic: bank.title };
    });
};
...
export const unansweredCount = (moduleId) =>
  (BANKS[moduleId]?.banks || []).reduce((n, b) => {
    const seen = new Map();
    return n + b.questions.filter((q) => {
      const nth = (seen.get(q.n) || 0) + 1;
      seen.set(q.n, nth);
      const key = `${moduleId}:${b.fid}:${q.n}${nth === 1 ? '' : `-${nth}`}`;
      return (q.unanswered || !q.answer.length) && !SUPPLIED[key];
    }).length;
  }, 0);
