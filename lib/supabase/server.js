 ...
 *
 * Once per request (React's cache): a page and the helpers it calls used to
 * ask separately. And the session is checked with getClaims(), which reads
 * the signed token locally rather than asking the auth server — the
 * middleware has already made that round trip for this request. On a project
 * still signing with the older shared secret, getClaims asks the server
 * itself, so it is never less safe than getUser, only sometimes faster.
 */
export const currentProfile = cache(async () => {
  const sb = await supabaseServer();
  let user = null;
  const { data: signed, error: unsigned } = await sb.auth.getClaims();
  const claims = signed?.claims;
  if (!unsigned && claims?.sub) {
