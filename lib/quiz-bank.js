...
export async function banksOf(moduleId) {
  ...
  // A refused read is not an empty subject. Fall back to the file rather than
  // draw an empty screen and let it read as "nothing here yet".
  if (error || !banks?.length) return file.banksFor(moduleId);
  ...
  if (refused) return file.banksFor(moduleId);
