...
export async function put(file, prefix = 'posts') {
  ...
  // The address is stored on the post, so what is already posted keeps
  // pointing where it was put. Nothing has to be moved for this to change.
  let url;
  if (r2.configured()) {
    url = await r2.put(path, file, { contentType: file.type });
  } else {
