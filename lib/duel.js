...
/**
 * Where a duel stands, for one of the two people in it.
 *
 * Every screen asks this — the list, the duel itself, the badge on الرئيسية —
 * so it is answered once, here, from the row. Two screens that work it out
 * separately are two screens that will one day disagree about whose turn it
 * is.
 *
 *   invited  somebody has challenged you and is waiting for an answer
 *   sent     you challenged somebody and they have not replied yet
 *   play     it is accepted and your half is not done
 *   waiting  your half is done and theirs is not
 *   done     both halves are in — the two scores can be shown
 *   refused  they said no
 */
export function stage(duel, me) {
  if (!duel || !me) return null;
  if (duel.state === 'refused') return 'refused';
  const mine = duel.challenger === me;
  if (duel.state === 'invited') return mine ? 'sent' : 'invited';
  const done = mine ? duel.challenger_at : duel.opponent_at;
  const theirs = mine ? duel.opponent_at : duel.challenger_at;
  if (!done) return 'play';
  if (!theirs) return 'waiting';
  return 'done';
}

/** The two things that are your move rather than theirs. */
export const myMove = (what) => what === 'invited' || what === 'play';

/**
 * What a challenger may choose before sending.
 *
 * Two dials, not ten. A duel that takes a minute to set up is a duel nobody
 * sends — and every option here changes the thing itself: how long it takes,
 * and whether it is a test of knowing or of knowing quickly.
 */
export const COUNTS = [5, 10, 20];
export const SECONDS = [0, 15, 30, 60];

/** Kept inside what is offered, whatever arrives in the request body. */
export const countOf = (n) => (COUNTS.includes(Number(n)) ? Number(n) : LENGTH);
export const secondsOf = (n) => (SECONDS.includes(Number(n)) ? Number(n) : 0);

/** « 30 ثانية لكل سؤال », or nothing at all when it is untimed. */
export const saysTime = (s) => (s ? `${s} ثانية لكل سؤال` : 'بلا وقت محدّد');
