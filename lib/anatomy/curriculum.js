...
/**
 * The regions a subject offers.
 *
 * A model belongs to the lecture it explains and is reached from that subject
 * and nowhere else — so ANATOMIE in PCEM2 S1 offers the ten head-and-neck
 * regions, and not the twenty-one files the app happens to hold.
 */
export const regionsFor = (promo, semester) =>
  REGIONS.filter((r) =>
    r.promo.toLowerCase() === String(promo || '').toLowerCase() &&
    r.semester.toUpperCase() === String(semester || '').toUpperCase());

/** The semester of a promo, by its `S1`/`S2` name. */
export const semesterOf = (promo, semester) => {
  const p = CURRICULUM.find((x) => x.promo.toLowerCase() === String(promo || '').toLowerCase());
  return p?.semesters.find((s) =>
    s.semester.toUpperCase() === String(semester || '').toUpperCase()
